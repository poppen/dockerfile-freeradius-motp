...
then
	...
	echo "reply:Reply-Message = 'No md5 or md5sum available on server'\n"
	exit 2
fi
...
if [ ! $# -eq 5 ] ; then
	echo "Auth-Type = REJECT, \n"
	echo "reply:Reply-Message = 'USAGE: otpverify.sh Username, OTP, Init-Secret, PIN, Offset'\n"
	exit 2
fi
...
chmod -R og-rxw /var/motp 2>/dev/null ||
	{
		echo "Auth-Type = REJECT, \n"
		echo "reply:Reply-Message = 'FAIL! Need write-access to /var/motp'\n"
		exit 2
	}
...
# delete old logins
find /var/motp/cache -type f -mmin +5 | xargs rm 2>/dev/null
